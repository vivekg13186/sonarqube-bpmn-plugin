...
@Rule(key = EventSubProcessTypedStartEventRule.RULE_KEY, name = "Event Sub Process Typed Start Event", description = "Ensures that start events inside event sub-processes are typed (have an event definition). This is required by the BPMN 2.0 standard.")

public class EventSubProcessTypedStartEventRule implements BpmnRule {
