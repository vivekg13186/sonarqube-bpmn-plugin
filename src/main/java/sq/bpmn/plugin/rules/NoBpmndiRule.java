...
public class NoBpmndiRule implements BpmnRule {
    public static final String RULE_KEY = "no-bpmndi";

    @Override
    public void check(BpmnModelInstance model, RuleReporter reporter) {
        if (!model.getModelElementsByType(BpmnDiagram.class).isEmpty()) {
            reporter.report("BPMNDI", "BPMN diagram interchange elements are not allowed");
        }
    }
